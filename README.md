...
Доработанное решение для задачи предсказания оценок книг пользователями в рамках соревнования НТО 2025/2026 (Профиль «Искусственный интеллект»). Использовался python 3.13
...
- Латентные факторы (SVD) и байесовские приоры (baseline-прогноз) для устойчивости на редких пользователях/книгах.
- Доп. предпочтения пользователя: доля прочитанных/отложенных по жанру, автору, языку, издателю; латенты по бинарным интеракциям (has_read 0/1).
...
Чтобы ускорить эксперименты без пересчёта тяжёлых текстовых признаков или лучше задействовать ресурсы машины (например, Mac M-серии), используйте флаги окружения:

```bash
# максимальная загрузка CPU/GPU: выставьте число потоков и устройство BERT
NUM_THREADS=12 BERT_DEVICE=mps BERT_BATCH_SIZE=24 poetry run python -m src.baseline.prepare_data

# только табличные признаки и TF-IDF (без BERT)
USE_BERT=0 poetry run python -m src.baseline.prepare_data
# полностью без текстовых признаков
USE_TFIDF=0 USE_BERT=0 poetry run python -m src.baseline.prepare_data
```
...
**Богатый набор признаков**. Добавлены:

- базовые табличные признаки: возрастные бины пользователей и «возраст книги»;
- устойчивые агрегаты: медианы/стандарты отклонений по пользователям/книгам, счётчики авторов, усреднения по языку и издателю, временные признаки (последняя оценка и давность активности пользователей/книг), а также показатели вовлечённости по всему `train.csv` (кол-во прочитанных, wishlist, отношение прочитанных к общим интеракциям, wishlist по книге);
- текстовые признаки: TF-IDF и (опционально) BERT-эмбеддинги, которые кэшируются на диск.
- ансамбль моделей: LightGBM + CatBoost с усреднением предсказаний для повышения стабильности.
- калибровка весов ансамбля по валидации и их автоматическая загрузка на этапе `predict`.
